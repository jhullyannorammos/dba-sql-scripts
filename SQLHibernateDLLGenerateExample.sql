...
drop table 'departamentos' referenced by a foreign key constraint 'FK_beql8npsmirygks0osp6cnang' on table 'mercadorias'.
    ...
    alter table cidades add constraint UK_a39enwbopuov4riwnn2io6wm unique (cidade);
    alter table colaboradores add constraint UK_kcy7wh8ac1lhmwurtvfvkec8s unique (cnh);
    alter table colaboradores add constraint UK_js76mkcfmyaqfvfq5b1a9tv2a unique (cpf);
    alter table colaboradores add constraint UK_ri9jj3oi860ua516fs7t0jmfd unique (email);
    alter table colaboradores add constraint UK_rr7sf7vw123rmtfcsuko1bw11 unique (matricula);
    alter table colaboradores add constraint UK_8micvps1uoufr7v6x1tki84qj unique (rg);
    alter table departamentos add constraint UK_mjl0kur7idqgwl2kx6c0yqttu unique (sigla);
    alter table estoque add constraint UK_2vq3dcwlwq7vhgso3sn35bxpi unique (modelo);
    alter table fornecedores add constraint UK_mx8fk9a906pfinmpc0f8gokrp unique (fornecedor);
    alter table licenses add constraint UK_rfc3upevs7fef0uv76jai4mgp unique (number_key);
    alter table licenses add constraint UK_h8smxutsdvgr8gitnbc6fwbfl unique (patrimonio_id);
    ...
    alter table patrimonios add constraint UK_emi67vt14fwsvqrpyy6fbok5x unique (patrimonio);
    alter table patrimonios add constraint UK_4sinyd44onkatn6191cpevu0 unique (mercadoria_id);
    alter table patrimonios add constraint UK_3h4kkre312o7ty968ya4dtbla unique (sl_id);
    alter table produtos add constraint UK_c57uc51cgsgkuynry8g2nuf74 unique (modelo_id);
...
insert into cidades(capital, cidade, federacao, sigla) values(?, ?, ?, ?);
insert into cidades(capital, cidade, federacao, sigla) values(?, ?, ?, ?);
insert into fornecedores (abrangencia, bloco, codigo_postal, cidade, complemento, logradouro, lote, numero, quadra, rua, tipo_endereco, uf, fornecedor, ddd, telefone, tipo_fone) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
insert into fornecedores(abrangencia, bloco, codigo_postal, cidade, complemento, logradouro, lote, numero, quadra, rua, tipo_endereco, uf, fornecedor, ddd, telefone, tipo_fone) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
insert into fornecedores(abrangencia, bloco, codigo_postal, cidade, complemento, logradouro, lote, numero, quadra, rua, tipo_endereco, uf, fornecedor, ddd, telefone, tipo_fone) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
insert into fornecedores(abrangencia, bloco, codigo_postal, cidade, complemento, logradouro, lote, numero, quadra, rua, tipo_endereco, uf, fornecedor, ddd, telefone, tipo_fone) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
insert into fornecedores(abrangencia, bloco, codigo_postal, cidade, complemento, logradouro, lote, numero, quadra, rua, tipo_endereco, uf, fornecedor, ddd, telefone, tipo_fone) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
insert into modelos(cor, dimensao, peso, fornecedor_id, lancamento, modelo) values(?, ?, ?, ?, ?, ?);
insert into modelos(cor, dimensao, peso, fornecedor_id, lancamento, modelo)  values(?, ?, ?, ?, ?, ?);
